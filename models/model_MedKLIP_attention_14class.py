...
class MedKLIP(nn.Module):
    ...
    def __init__(self, config, ana_book, disease_book, mode='train'):
        ...
        self.classifier_attention = nn.Parameter(torch.empty(config['num_classes'],4,1))
        torch.nn.init.normal_(self.classifier_attention, mean=0.0, std=0.02)
    ...
    def forward(self, images,labels,smaple_index = None, is_train = True, no_cl= False, exclude_class= False):
        ...
        # attention reweight
        features = [i.transpose(0,1).unsqueeze(-1) for i in features]
        feature = torch.cat(features, dim=-1) # b,14,768,4
        fB = feature.shape[0] # b
        fC = feature.shape[1] # 14
        
        ca = F.softmax(self.classifier_attention, dim=1)
        ca = ca.repeat(fB,1,1,1) # b,14,4,1
        ca = rearrange(ca,'b c m l -> (b c) m l') # b*14,4,1

        feature = rearrange(feature,'b c d m -> (b c) d m') # b*14,768,4
        out_feature = torch.bmm(feature, ca).squeeze() # b*14,768
        out_feature = rearrange(out_feature,'(b c) d-> b c d',b=fB,c=fC) # b, 14, 768

        out_feature = out_feature.transpose(0,1)
