...
from models.model_MedKLIP_loss_out import MedKLIP as MedKLIP 
from models.model_MedKLIP_14class_loss_out import MedKLIP as MedKLIP_14
...
def train(model, image_encoder, text_encoder, tokenizer, data_loader, optimizer, epoch, warmup_steps, device, scheduler, config):
    clip_loss = ClipLoss()
    ...
    for i, sample in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if config['no_cl'] == False:
            logits, ll, cl_labels = model(image_features,cur_text_features,cur_ana_features,index)
        else:
            logits = model(image_features,cur_text_features,cur_ana_features,index)
...
def valid(model, image_encoder, text_encoder, tokenizer, data_loader, device,config):
    clip_loss = ClipLoss()
    global_local_loss = GlobalLocalLoss() if 'global_local_loss' in config and config['global_local_loss'] else None
    model.eval()
    if config['4_image_encoder']:
...
def main(args, config):
    ...
    if config['model_type']== 'resnet':
        image_encoder = [ModelRes(config).to(device),ModelRes(config).to(device),ModelRes(config).to(device),ModelRes(config).to(device)] if config['4_image_encoder'] else ModelRes(config).to(device)
    elif config['model_type'] == 'densenet':
        image_encoder = [ModelDense(config).to(device),ModelDense(config).to(device),ModelDense(config).to(device),ModelDense(config).to(device)] if config['4_image_encoder'] else ModelDense(config).to(device)
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        train_stats, tensorboard_train = train(model, image_encoder, text_encoder, tokenizer, train_dataloader, optimizer, epoch, warmup_steps, device, lr_scheduler, config) 

        writer.add_scalar('lr/leaning_rate',  lr_scheduler._get_lr(epoch)[0] , epoch)

        tensorboard_val = valid(model, image_encoder, text_encoder, tokenizer, val_dataloader, device, config)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='/home/ps/leijiayu/CODE/MedKLIP/Pretrain_MedKLIP_bce/configs/Pretrain_MedKLIP_kad_sc_glrep_morecls.yaml')
    parser.add_argument('--finetune_checkpoint', default='')
    parser.add_argument('--output_dir', default='outputdir_34_kad_sc_glrep_morecls_tensor')
    parser.add_argument('--device', default='cuda')
    parser.add_argument('--gpu', type=str,default='3,0,1,2', help='gpu')
