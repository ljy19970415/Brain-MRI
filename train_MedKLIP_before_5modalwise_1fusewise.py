...
def train(model, image_encoder, text_encoder, fuseModule1,fuseModule2, tokenizer, data_loader, optimizer, epoch, warmup_steps, device, scheduler, config):
    clip_loss = ClipLoss()
    ...
    for i, sample in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        # before fuse
        fuse_image_feature, fuse_image_feature_pool = fuseModule1(image_features)
        image_features_pool.append(fuse_image_feature_pool)
        
        image_features.append(fuse_image_feature)
        fuse_image_feature2, fuse_image_feature_pool2 = fuseModule2(image_features)
        image_features_pool.append(fuse_image_feature_pool2)

        if config['no_cl'] == False:
            logits, ll, cl_labels = model(fuse_image_feature2,cur_text_features,cur_ana_features,index)
        else:
...
def valid(model, image_encoder, text_encoder, fuseModule1,fuseModule2, tokenizer, data_loader, device,config):
    clip_loss = ClipLoss()
    global_local_loss = GlobalLocalLoss() if 'global_local_loss' in config and config['global_local_loss'] else None
    model.eval()
    if config['4_image_encoder']:
...
def main(args, config):
    ...
    fuseModule1 = beforeFuse(config,4).to(device) # before fusion
    fuseModule2 = beforeFuse(config,5).to(device)
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        train_stats, tensorboard_train = train(model, image_encoder, text_encoder, fuseModule1,fuseModule2, tokenizer, train_dataloader, optimizer, epoch, warmup_steps, device, lr_scheduler, config) 

        writer.add_scalar('lr/leaning_rate',  lr_scheduler._get_lr(epoch)[0] , epoch)

        tensorboard_val = valid(model, image_encoder, text_encoder, fuseModule1,fuseModule2,tokenizer, val_dataloader, device, config)
        ...
        if utils.is_main_process():  
            ...
            save_obj = {
                ...
                'fuseModule1': fuseModule1.state_dict(),
                'fuseModule2': fuseModule2.state_dict(),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='/home/ps/leijiayu/CODE/MedKLIP/Pretrain_MedKLIP_bce/configs/Pretrain_MedKLIP_lorep_lesscls_fusereport_nocl.yaml')
    parser.add_argument('--finetune_checkpoint', default='')
    parser.add_argument('--output_dir', default='outputdir_34_kad_sc_lorep_lesscls_beforefuse_5modalwise_1fusewise_nocl')
    parser.add_argument('--device', default='cuda')
    parser.add_argument('--gpu', type=str,default='2,3,0,1', help='gpu')
